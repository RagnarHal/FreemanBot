...
const commands = [
  ...
  {
    command: 'roll <n>',
    description: 'I\'ll roll an n-sided die. I\'ll roll a 100-sided die if no argument is specified'
  },
  {
    command: 'hug',
...
export default message => {
  message.delete().catch(() => logger.warn('say: Unable to delete message, do I have permission?'));
  const msg =
`Hi, I'm FreemanBot! Here are my available commands:

${commands.map(c => `__**${config.prefix}${c.command}**__\n${c.description}`).join('\n\n')}
