...
async function getCurrentTriviaTime() {
  const { rows } = await database
    .getClient()
    .query("SELECT * FROM status WHERE vname='trivia_time'");
...
async function getCurrentTriviaKey() {
  const { rows } = await database
    .getClient()
    .query("SELECT * from status WHERE vname='trivia_key'");
...
async function getTriviaById(id) {
  const { rows } = await database
    .getClient()
    .query("SELECT * from trivia_questions_and_answers WHERE id = $1;", [id]);
...
async function getUserScore(userId) {
  const { rows } = await database
    .getClient()
    .query("SELECT * FROM trivia_scores WHERE id=$1", [userId]);
...
function setCurrentTriviaKey(triviaKey) {
  return database
    .getClient()
    .query("UPDATE status SET vint=$1 WHERE vname='trivia_key'", [triviaKey]);
}

function setCurrentTriviaTime(triviaTime) {
  return database
    .getClient()
    .query("UPDATE status SET vint=$1 WHERE vname='trivia_time'", [triviaTime]);
}

export async function getTriviaHintVoteCount() {
  const res = await database
    .getClient()
    .query("SELECT COUNT(*) FROM trivia_scores WHERE vote_hint=1");

  return res.rows[0].count;
}

export function resetTriviaHintVoteCount() {
  return database.getClient().query("UPDATE trivia_scores SET vote_hint=0");
}
...
export async function beginNewRound() {
  ...
  try {
    await database.getClient().query("BEGIN");
    await Promise.all([
    ...
    await database.getClient().query("COMMIT");
    ...
    await database.getClient().query("ROLLBACK");
    throw err;
  }
}
...
export async function awardTriviaPoints(userId, userName, points) {
  ...
  const { rows } = await database
    .getClient()
    .query(queryText, [userId, userName, points]);
...
export async function voteTriviaSkip(userId, userName) {
  ...
  await database.getClient().query(queryText, queryParams);

  const skipCount = await database
    .getClient()
    .query("SELECT COUNT(*) FROM trivia_scores WHERE vote_skip=1");

  return skipCount.rows[0].count;
}

export function resetTriviaSkip() {
  return database.getClient().query("UPDATE trivia_scores SET vote_skip=0");
}

export function markTriviaNeedingHints(id, mark) {
  return database
    .getClient()
    .query("UPDATE trivia_questions_and_answers SET mark=$2 WHERE id=$1", [
      id,
      mark
    ]);
}
...
export function setTriviaHints(userId, hints) {
  return database
    .getClient()
    .query("UPDATE trivia_questions_and_answers SET hints=$2 WHERE id=$1", [
      userId,
      hints
    ]);
}
...
export async function getTriviaHintLevel() {
  const res = await database
    .getClient()
    .query("SELECT vint FROM status WHERE vname='trivia_hint_level'");
...
export function resetTriviaHintLevel() {
  return database
    .getClient()
    .query("UPDATE status SET vint=0 WHERE vname='trivia_hint_level'");
}

export function voteIncreaseTriviaHintLevel(userId) {
  return database
    .getClient()
    .query("UPDATE trivia_scores SET vote_hint=1 WHERE id=$1", [userId]);
}
